...
def load_admin4cod(source):
    ...
    query = f"CREATE TABLE IF NOT EXISTS admincod AS (select * EXCLUDE geometry, ST_GeomFromWKB(geometry) AS geometry from {from_query})"
    print(query)
    conn.execute(query)
...
def process_input(conn, input_data, intersection_method="centroid"):
    ...
    gdf = gdf.to_wkb()
    create_table_query = f"CREATE TABLE IF NOT EXISTS input_data AS (select * EXCLUDE geometry, ST_GeomFromWKB(geometry) AS geometry from gdf)"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process spatial data with DuckDB.")
    ...
    parser.add_argument(
        "--method", default="centroid", choices=["centroid"], help="Intersection method"
    )
